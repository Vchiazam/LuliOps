...
export const passwordResetValidateInSchema = z.object({
  token: z.string(),
  uid: z.string(),
});
export const passwordResetConfirmInSchema =
  passwordResetValidateInSchema.extend({ new_password: z.string() });
...
export const monitorInSchema = z.object({
  ...
  target: z.string(),
  is_active: z.boolean(),
  current_state: monitorStateEnum.optional(),
});
...
export const pagedMonitorStateHistoryOutSchema = pagedSchema(
  monitorStateHistoryOutSchema,
);
